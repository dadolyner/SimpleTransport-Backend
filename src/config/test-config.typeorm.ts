    ...
    imports: [
        ...
        TypeOrmModule.forRootAsync({
            ...
            useFactory: (configService: ConfigService) => ({
                type: 'postgres',
                host: configService.get<string>('TEST_DATABASE_HOST'),
                port: +configService.get<number>('TEST_DATABASE_PORT'),
                username: configService.get<string>('TEST_DATABASE_USER'),
                password: configService.get<string>('TEST_DATABASE_PASSWORD'),
                database: configService.get<string>('TEST_DATABASE_NAME'),
                entities: [Users, Rentals, Vehicles, Fuels, Models, Brands, Postals, Places, Countries, Colors],
                synchronize: true,
                // logging: true,
